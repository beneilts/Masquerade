	...
	if TRIGGER then
		...
		for i,other in ipairs(overlaps) do
			...
			if removeAmount > 0 then
				local denominator = distance * removeAmount * 0.5
...
function EngageNearest()
	...
	local enemy = Game.FindNearestPlayer(ROOT:GetWorldPosition(), {ignoreDead=true, ignorePlayers = _G.playerIgnoreList})--FindNearestEnemy()
	if enemy then
		SetTarget(enemy)
